...
def sort_repos(repo_list):
    ...
    if repo_list == []:
        ...
        pivot = repo_list[0]
        lesser = sort_repos([repo for repo in repo_list[1:]
                             if repo.stargazers_count < pivot.stargazers_count
                             ])
        greater = sort_repos([repo for repo in repo_list[1:]
                             if repo.stargazers_count >= pivot.stargazers_count
                              ])
...
def main(username='defunkt'):
    ...
    print("%s(%s)'s most popular repositories by stargazers count are:" %
